...
class OptimizedC45DecisionTree:
    def __init__(self):
        self.tree = None
        self.level = 0  # 用于缩进显示
    ...
    def calc_split_info(self, X, feature, print_details=True):
        """计算分裂信息"""
        ...
        if print_details:
            indent = "  " * self.level
            print(f"{indent}计算分裂信息:")

        for val in unique_vals:
            subset_size = len(X[X[feature] == val])
            ...
            if print_details:
                ...
                print(f"{indent}    样本数: {subset_size}")
                print(f"{indent}    比例: {p:.4f}")

        if print_details:
            print(f"{indent}分裂信息 = {split_info:.4f}")
    ...
    def _build_tree(self, X, y):
        ...
        for feature in X.columns:
            gain_ratios[feature] = self.calc_gain_ratio(X, y, feature)
        ...
        tree = {'feature': best_feature, 'gain_ratio': gain_ratios[best_feature]}
        ...
        for value in np.unique(X[best_feature]):
            ...
            val_name = "是" if value == 1 else "否" if value == 0 else val
...
def plot_optimized_tree(tree, ax):
    ...
    # 设置图形属性
    ax.set_title('Optimized C4.5 Decision Tree', pad=20, size=14)
    ax.set_xlim(-5, 5)
    ax.set_ylim(0, 4)
    ax.axis('off')
...
def main():
    ...
    # 训练C4.5决策树
    c45_tree = OptimizedC45DecisionTree()
    c45_tree.fit(X, y)

    # 预测和计算准确率
    y_pred = c45_tree.predict(X)
    accuracy = accuracy_score(y, y_pred)

    # 创建决策树可视化
    fig, ax = plt.subplots(figsize=(12, 8))
    plot_optimized_tree(c45_tree.tree, ax)

    plt.savefig('optimized_c45_decision_tree.png', dpi=300, bbox_inches='tight',
